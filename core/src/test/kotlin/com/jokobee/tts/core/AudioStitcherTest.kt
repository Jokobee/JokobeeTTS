...
class AudioStitcherTest {
    ...
    @Test fun singleSegment_passthrough() {
        ...
        assertSame(a, AudioStitcher.stitch(listOf(a)))   // retourné tel quel
    }
    ...
    @Test fun twoSegments_silenceInserted() {
        ...
        assertEquals(500 + 7200 + 500, out.size)          // 300 ms @ 24k = 7200 échantillons
    ...
    @Test fun crossfade_noClicks() {
        ...
        val out = AudioStitcher.stitch(listOf(a, b), cfg)
        assertTrue("bord de début fondu", abs(out[0]) < 0.01f)
        assertTrue("bord de fin de segment fondu", abs(out[1999]) < 0.01f)  // dernier éch. de a
        assertTrue("dernier éch. global fondu", abs(out[out.size - 1]) < 0.01f)
    }
    ...
    @Test fun peakNormalize_disabled() {
        ...
        assertEquals(0.5f, peak(out), 1e-6f)              // signal non modifié
    }
    ...
    @Test fun configurable_silenceMs_zero() {
        ...
        assertEquals(1000, out.size)                      // pas de silence
    }
